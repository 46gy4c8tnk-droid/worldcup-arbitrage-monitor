...
class Engine:
        ...
        with self.lock:
            for ev in events:
                ...
                opp = {
                    ...
                    "legs": [
                        {"outcome": o, "bookmaker": b, "odds": odds}
    ...
    def _maybe_bet(self, ev, best, inv, profit_pct):
        ...
        # 按 1/odds 比例分配注金，使各结果赔付一致（取整后按最小赔付计）
        legs = []
        for outcome, (bookie, odds) in best.items():
            stake = round(total * (1 / odds) / inv, 2)
            legs.append((outcome, bookie, odds, stake))
        total_stake = round(sum(l[3] for l in legs), 2)
        payout = round(min(l[2] * l[3] for l in legs), 2)
        if payout <= total_stake:
            return "取整后无利润"

        group_id = uuid.uuid4().hex[:10]
        self.db.execute(
            "INSERT INTO arb_groups VALUES(?,?,?,?,?,NULL,?,?,?,'open')",
            (group_id, ev["event_id"], ev["match_name"], ev["commence_ts"],
             time.time(), round(profit_pct, 3), total_stake, payout),
        )
        ...
        self._set_meta("balance", round(balance - total_stake, 2))
        self.db.commit()
        self._log("bet",
                  f"下单 {ev['match_name']}：投入 {total_stake:.2f}，"
                  f"锁定利润 {payout - total_stake:.2f}（{profit_pct:.2f}%）")
        self._record_equity(f"下单 {ev['match_name']}")
        self._check_reset()
        return "已下单"
